...
class environment:
    def __init__(self):
        ...
        self.pyboy = PyBoy(filename, window_type="headless" if quiet else "SDL2", window_scale=3, debug=quiet,
                           game_wrapper=True)
    ...
    for network in networks:
        ...
        for act in actions:
            ...
            try:
                ...
                percentuale = int(env.mario.level_progress * 100 / total)

            except:
                break
            print('ACT: ' + str(numeroMossa))
        ...
        print('Position Total: {}'.format(network.position))
        print('Ultima Mossa: {}'.format(network.mossa))
...
def mutate(networks):
    for network in networks:
        for idx in range(0,int(games)):
            val = np.random.uniform(0, 1)
            if val <= 1: #mutation chance
                network.actions[idx] = 4
...
"""
def mutate(networks):
    percentuale = int(networks[0].position * 100 / total)
    for network in networks[:1]:
        tempo = network.mossa - 50
        nuovo = []
        vecchio = []
        for idx in range(tempo, int(games)):
            val = np.random.uniform(0, 1)
            if val <= 0.5:  # mutation chance
                vecchio.append(network.actions[idx])
                print("NUOVO" + str(network.actions[idx]))
                network.actions[idx] = np.random.randint(4, 4)
                nuovo.append(network.actions[idx])
                print("NUOVO" + str(network.actions[idx]))
        print(vecchio)
        print(nuovo)
    return networks
